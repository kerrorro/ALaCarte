...
import KEYBOARD from './keyboard';
	...
	onEnter(label) {
		if(label.name != "field"){
			// Clears label upon search
			if (label.selectionLength == 0){				label.select(0, label.length)
			}			label.insert()
			application.distribute("toggleHighlight", false);
		} else {	
			KEYBOARD.hide();	
		}
